...
{
    public partial class adminParticipantsTennis : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Student_Sport where SportName='Tennis'";
            }
            else
            {
                qry = "Select * from Student_Sport where SportName= 'Tennis' AND FirstName like '%" + searchText + "%'";
            }
        ...
        protected void Button1_Click1(object sender, EventArgs e)//cancel alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select Email,FirstName, Surname from Student_Sport where SportName= 'Tennis' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "Tennis practices have been cancelled. Please check the practice schedules for more information.";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Tennis Practices have been cancelled";
