...
{
    public partial class adminParticipantathletics : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Student_Sport where SportName='Athletics'";
            }
            else
            {
                qry = "Select * from Student_Sport where SportName= 'Athletics' AND FirstName like '%" + searchText + "%'";
            }
        ...
        //        SqlCommand cmd = new SqlCommand("SELECT * FROM Student_Sport where Sport_Name='Athletics'", con);
        ...
        //    }
        //}
        //protected void Button1_Click(object sender, EventArgs e)//email
        //{
        //    getemail();
        //}
        ...
        protected void Button1_Click1(object sender, EventArgs e)//cancel alerts
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select Email,FirstName, Surname from Student_Sport where SportName= 'Athletics' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "Athletic practices have been cancelled. Please check announcements and the practice schedules for more information.";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Athletic Practices have been cancelled";
                    mm.Body = "Dear " + fname + " "+ surname + ",<br /><br />" + message + "<br /><br/>" + "Regards" + ", <br />" + "Admin";
                    mm.IsBodyHtml = true;
                    SmtpClient smtp = new SmtpClient();
                    smtp.Host = "smtp.gmail.com";
                    smtp.Port = 587;
