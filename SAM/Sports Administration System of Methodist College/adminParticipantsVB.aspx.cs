...
{
    public partial class adminParticipantsVB : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Student_Sport where SportName='Volleyball'";
            }
            else
            {
                qry = "Select * from Student_Sport where SportName= 'Volleyball' AND FirstName like '%" + searchText + "%'";
            }
        ...
        //        SqlCommand cmd = new SqlCommand("SELECT * FROM Student_Sport where Sport_Name='Volleyball'", con);
        ...
        protected void Button1_Click1(object sender, EventArgs e)//Cancel alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select Email,FirstName, Surname from Student_Sport where SportName= 'Volleyball' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "Volleyball practices have been cancelled. Please check the practice schedules for more information.";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Volleyball Practices have been cancelled";
