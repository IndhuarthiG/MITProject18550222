...
{
    public partial class adminChessroster : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Rosters where SportName='Chess'";
            }
            else
            {
                qry = "Select * from Rosters where SportName= 'Chess' AND Surname like '%" + searchText + "%'";
            }
        ...
        protected void btnSendMail_Click(object sender, EventArgs e)//roster alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select EmailID,Title, Surname, Date, Time from Rosters where SportName= 'Chess' AND Status='Active' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "You have been rostered for Chess practices this month ";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Chess sports roster";
        ...
        protected void Button1_Click(object sender, EventArgs e)//cancel alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select EmailID,Title, Surname, Date, Time from Rosters where SportName= 'Chess' AND Status='Cancelled' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "Chess practices have been cancelled. ";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
        ...
        {
            Response.Redirect("adminchessavail.aspx");
        }
