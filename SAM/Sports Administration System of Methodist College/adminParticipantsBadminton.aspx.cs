...
{
    public partial class adminParticipantsBadminton : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Student_Sport where SportName='Badminton'";
            }
            else
            {
                qry = "Select * from Student_Sport where SportName= 'Badminton' AND FirstName like '%" + searchText + "%'";
            }
        ...
        //        SqlCommand cmd = new SqlCommand("SELECT * FROM Student_Sport where Sport_Name='Badminton'", con);
        ...
        protected void Button1_Click(object sender, EventArgs e)//cancel alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select Email,FirstName, Surname from Student_Sport where SportName= 'Badminton' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "Badminton practices have been cancelled. Please check the practice schedules for more information.";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Badminton Practices have been cancelled";
