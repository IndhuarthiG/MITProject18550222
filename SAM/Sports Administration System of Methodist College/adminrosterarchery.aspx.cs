...
{
    public partial class adminrosterarchery : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Rosters where SportName='Archery'";
            }
            else
            {
                qry = "Select * from Rosters where SportName= 'Archery' AND Surname like '%" + searchText + "%'";
            }
        ...
        protected void btnSendMail_Click(object sender, EventArgs e)//roster alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select EmailID,Title, Surname, Date, Time from Rosters where SportName= 'Archery' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "You have been rostered for archery practices this month";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Archery sports roster";
        ...
        protected void Button1_Click(object sender, EventArgs e)//cancellation alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select EmailID,Title, Surname, Date, Time from Rosters where SportName= 'Archery' AND Status='Cancelled' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "Archery practices have been cancelled";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
        ...
        {
            Response.Redirect("adminArchAvail.aspx");
        }
