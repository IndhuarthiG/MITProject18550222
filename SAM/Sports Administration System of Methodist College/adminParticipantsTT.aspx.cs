...
{
    public partial class adminParticipantsTT : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Student_Sport where SportName='Table Tennis'";
            }
            else
            {
                qry = "Select * from Student_Sport where SportName= 'Table Tennis' AND FirstName like '%" + searchText + "%'";
            }
        ...
        //        SqlCommand cmd = new SqlCommand("SELECT * FROM Student_Sport where Sport_Name='Table Tennis'", con);
        ...
        protected void Button1_Click1(object sender, EventArgs e)//cancel alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select Email,FirstName, Surname from Student_Sport where SportName= 'Table Tennis' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "Table Tennis practices have been cancelled. Please check the practice schedules for more information.";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Table Tennis Practices have been cancelled";
