...
{
    public partial class adminParticipantsBB : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Student_Sport where SportName='Basketball'";
            }
            else
            {
                qry = "Select * from Student_Sport where SportName= 'Basketball' AND FirstName like '%" + searchText + "%'";
            }
        ...
        //        SqlCommand cmd = new SqlCommand("SELECT * FROM Student_Sport where Sport_Name='Basketball'", con);
        ...
        protected void Button1_Click(object sender, EventArgs e)// cancel alerts
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select Email,FirstName, Surname from Student_Sport where SportName= 'Basketball' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "Basketball practices have been cancelled. Please check the practice schedules for more information.";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Basketball Practices have been cancelled";
