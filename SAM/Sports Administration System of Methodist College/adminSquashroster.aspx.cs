...
{
    public partial class adminSquashroster : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Rosters where SportName='Squash'";
            }
            else
            {
                qry = "Select * from Rosters where SportName= 'Squash' AND Surname like '%" + searchText + "%'";
            }
        ...
        protected void btnSendMail_Click(object sender, EventArgs e)//roster alerts
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select EmailID,Title, Surname, Date, Time from Rosters where SportName= 'Squash' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "You have been rostered for Squash practices this month";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Squash sports roster";
        ...
        protected void Button1_Click(object sender, EventArgs e)//cancel alerts
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select EmailID,Title, Surname, Date, Time from Rosters where SportName= 'Squash' AND Status='Cancelled' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "Squash practices have been cancelled";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
        ...
        {
            Response.Redirect("adminsqavail.aspx");
        }
