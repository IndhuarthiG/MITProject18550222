...
{
    public partial class adminParticipantsChess : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Student_Sport where SportName='Chess'";
            }
            else
            {
                qry = "Select * from Student_Sport where SportName= 'Chess' AND FirstName like '%" + searchText + "%'";
            }
        ...
        //        SqlCommand cmd = new SqlCommand("SELECT * FROM Student_Sport where Sport_Name='Chess'", con);
        ...
        protected void Button1_Click(object sender, EventArgs e)//Cancel alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select Email,FirstName, Surname from Student_Sport where SportName= 'Chess' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "Chess practices have been cancelled. Please check the practice schedules for more information.";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Chess Practices have been cancelled";
