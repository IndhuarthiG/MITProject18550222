...
{
    public partial class adminBadminroster : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Rosters where SportName='Badminton'";
            }
            else
            {
                qry = "Select * from Rosters where SportName= 'Badminton' AND Surname like '%" + searchText + "%'";
            }
        ...
        protected void btnSendMail_Click(object sender, EventArgs e)//roster alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select EmailID,Title, Surname, Date, Time from Rosters where SportName= 'Badminton' AND Status='Active' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "You have been rostered for Badminton practices this month ";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Badminton sports roster";
        ...
        protected void Button1_Click(object sender, EventArgs e)//cancellation alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select EmailID,Title, Surname, Date, Time from Rosters where SportName= 'Badminton' AND Status='Cancelled' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "Badminton practices have been cancelled. ";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
        ...
        protected void Button2_Click1(object sender, EventArgs e)//check btn
        {
            Response.Redirect("adminBadminavailable.aspx");
        }
    }
}