...
{
    public partial class adminBBroster : System.Web.UI.Page
    {
        ...
        private void GetData(string searchText = "")
        {
            ...
            {
                qry = "Select * from Rosters where SportName='Basketball'";
            }
            else
            {
                qry = "Select * from Rosters where SportName= 'Basketball' AND Surname like '%" + searchText + "%'";
            }
        ...
        protected void btnSendMail_Click(object sender, EventArgs e)//roster alert
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select EmailID,Title, Surname, Date, Time from Rosters where SportName= 'Basketball' AND Status='Active' ", con);
                ...
                while (dr.Read())
                {
                    ...
                    string message = "You have been rostered for Basketball practices this month ";
                    MailMessage msg = new MailMessage();
                    msg.To.Add(add);
                    msg.From = addr;
                    MailMessage mm = new MailMessage(addr, add);
                    mm.Subject = "Basketball sports roster";
        ...
        protected void Button1_Click(object sender, EventArgs e)//cancel alert
        {
            try
            {
                ...
                SqlConnection con = new SqlConnection(strcon);
                con.Open();
                SqlCommand cmd = new SqlCommand("select EmailID,Title, Surname, Date, Time from Rosters where SportName= 'Basketball' AND Status='Cancelled' ", con);
                    ...
                    while (dr.Read())
                    {
                        ...
                        string message = "Basketball practices have been cancelled. ";
                        MailMessage msg = new MailMessage();
                        msg.To.Add(add);
                        msg.From = addr;
                        MailMessage mm = new MailMessage(addr, add);
                        mm.Subject = "Sports Practices Cancelled";
        ...
        {
            Response.Redirect("adminavailableBBTeachers.aspx");
        }

        //protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)//colour row
        //{
        ...
        //        //{
        //        //    if (e.Row.Cells[3].Text == DateTime.Now.ToString())
        //        //    {
        ...
        //        //DateTime expdate = Convert.ToDateTime(e.Row.Cells[3].Text.Trim());
